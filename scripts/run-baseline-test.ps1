...
Write-Host "Checking baseline gateway health..."
try {
    Invoke-WebRequest -UseBasicParsing http://127.0.0.1:8080/health | Select-Object StatusCode, Content
} catch {
    Write-Warning "Health request failed. Make sure backend and baseline Nginx are running."
}

Write-Host "Starting local Slowloris simulation against baseline config..."
